...
# -*- coding: utf-8 -*-
"""
Датасет заболеваний собак с учетом частоты и лабораторных анализов
...
class DogDiseaseDataset:
    """
    Создание датасета заболеваний собак с учетом частоты и специфики
    """
    ...
    def __init__(self):
        ...
        # Веса частоты заболеваний собак (чем больше, тем чаще встречается)
        self.disease_frequency_weights = {
            # Очень частые заболевания
            'парвовирусный_энтерит': 0.20,
            'чума_плотоядных': 0.18,
            'гастроэнтерит': 0.15,
            'дерматит': 0.12,
            'пиодермия': 0.10,
            'конъюнктивит': 0.12,
            ...
            'кариес': 0.08,
            'абсцесс': 0.08,
            'дисплазия_тазобедренного_сустава': 0.08,
            'артрит': 0.10,
            'ожирение': 0.10,
            'диабет': 0.08,
            'гипотиреоз': 0.08,
            'болезнь_кушинга': 0.05,
            'анорексия': 0.06,
            ...
            'цистит': 0.10,
            'пиелонефрит': 0.08,
            'дилатационная_кардиомиопатия': 0.08,
            'эндокардит': 0.06,
            'аритмия': 0.08,
            'эпилепсия': 0.08,
            'менингит': 0.05,
            ...
            'меланома': 0.04,
            'гипертиреоз': 0.06,
            'болезнь_аддисона': 0.04,
            'булимия': 0.04,
        }
    def create_comprehensive_symptoms(self):
        """Создание полного списка симптомов для собак"""
        self.symptoms = [
            # Общие симптомы
        ...
        print(f"Создано {len(self.symptoms)} симптомов для собак")
    def _assign_symptoms_for_disease(self, disease, symptoms_vector):
        """Назначение симптомов для конкретного заболевания собаки"""
        ...
        # Пироплазмоз
        elif disease == 'пироплазмоз':
            symptoms_vector[self.symptoms.index('лихорадка')] = 1
            symptoms_vector[self.symptoms.index('слабость')] = 1
            symptoms_vector[self.symptoms.index('анорексия')] = 1
            symptoms_vector[self.symptoms.index('бледность_слизистых')] = 1
            symptoms_vector[self.symptoms.index('желтуха')] = 1
            symptoms_vector[self.symptoms.index('темная_моча')] = 1
            symptoms_vector[self.symptoms.index('lab_rbc_ниже_нормы')] = 1
            symptoms_vector[self.symptoms.index('lab_hgb_ниже_нормы')] = 1
            symptoms_vector[self.symptoms.index('lab_bilirubin_total_выше_нормы')] = 1
        ...
        # Демодекоз
            ...
            symptoms_vector[self.symptoms.index('корочки')] = 1
        ...
        # Пиодермия
        elif disease == 'пиодермия':
            symptoms_vector[self.symptoms.index('зуд')] = 1
            symptoms_vector[self.symptoms.index('покраснение_кожи')] = 1
            symptoms_vector[self.symptoms.index('язвы')] = 1
            symptoms_vector[self.symptoms.index('корочки')] = 1
            symptoms_vector[self.symptoms.index('выпадение_шерсти')] = 1
            symptoms_vector[self.symptoms.index('неприятный_запах')] = 1
        ...
        elif disease == 'дисплазия_тазобедренного_сустава':
    ...
    def create_weighted_dataset(self, n_samples=30000):
        """Создание взвешенного датасета для собак"""
        print(f"Создание датасета для собак с {n_samples} образцами...")
